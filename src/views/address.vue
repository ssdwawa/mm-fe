<template>
  <div class="checkout-page">
    ...
    <div class="container">
      <div class="checkout-addr">
        ...
        <div class="addr-list-wrap">
          <div class="addr-list">
            <ul class="choice-li">
              <li  v-for="(item,index) in addressList" @click="choice(item,index)" >
                <dl>
                  <dt>{{item.sheng}}</dt>
                  <dd class="address">{{item.addressprov}}</dd>
                  <dd class="address">{{item.addressshi}}</dd>
                  <dd class="tel">{{item.addressaccute}}</dd>
                  <dd class="tel">{{item.phone}}</dd>
                </dl>
                <div class="addr-opration addr-del">
                  <a href="javascript:;" class="addr-del-btn" @click="deleteAddress(item)">
    ...
    <modal v-bind:mdShow="mdCartShow" v-on:close="closeModal">
      ...
      <div slot="address">
        <p>
          <label>省份：</label><input type="text" value=""  v-model="sheng">
        </p>
        <p>
          <label>城市：</label><input type="text" value=""  v-model="shi">
        </p>
        <p>
          <label>街道：</label><input type="text" value=""  v-model="jie">
        </p>
        <p>
          <label>电话：</label><input type="text" value=""  v-model="phone">
        </p>

      </div>
      <div slot="btn-group">
        <a href="javascript:;" @click="addPut()" >确认提交</a>
      </div>
    </modal>
  </div>
</template>
...
export default {
  ...
  methods: {
    loadAddress:function(){
      axios.post('/users/loadAddress',{
         userId:_tool.getCookie('userId')
      }).then((res)=>{
    ...
    addPut:function(){
       axios.post('/users/addAddress',{
        userId:_tool.getCookie('userId'),
        addressprov:this.sheng,
        addressshi:this.shi,
        addressaccute:this.jie,
        phone:this.phone
      }).then((res)=>{
        this.mdCartShow=false
        if(res.data.status==1){
    ...
    choice:function(item,index){
      $('.choice-li li').eq(index).addClass('choice').siblings().removeClass('choice');
